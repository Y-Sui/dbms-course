...
import filetest.BlkID;
import filetest.Page;
/**该类用于处理目录创建、文件读写、追加块及文件
 * 需要占用的块数/号等
 */
 ...
 * 可以指定数据文件的路径dbdirname和块大小blksize*/
	public File2(String dbdirname,int blksize) {
   ...
   public synchronized void read(BlkID blk, Page p) {
      try {
         RandomAccessFile f = getFile(blk.fileName());
         f.seek(blk.blkNum() * blksize);
         f.getChannel().read(p.contents());
      }catch (IOException e) {
         throw new RuntimeException("不能读取块block(blk)" + blk);
      }
   }/**把要写入的内容先通过逻辑块blk装入内存页p，再进行写入*/
   public synchronized void write(BlkID blk, Page p) {
      try {
         RandomAccessFile f = getFile(blk.fileName());
         f.seek(blk.blkNum() * blksize);
   ...
   public synchronized BlkID append(String filename) {
      int newblknum = fileblkNum(filename);
      BlkID blk = new BlkID(filename, newblknum);
